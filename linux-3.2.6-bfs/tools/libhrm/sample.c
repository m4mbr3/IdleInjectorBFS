...
main(int argc, char *argv[])
{
	...
	hrm_attach(&monitor1, 2, false);
	printf("Attached to gid %d\n", 2);
	for (i = 0; i < windows_number; i++)
		hrm_set_goal(&monitor1, ws[i], min[i], max[i]);

	windows_number = hrm_get_windows_number(&monitor1);
	printf("\t%d MA set", windows_number);
	if (windows_number) {
		printf(" of sizes: ");
		for (int i = 1; i <= HRM_MAX_WINDOWS; i++) {
			*ws = hrm_get_window_size(&monitor1, i);
			if (*ws > 0 && *ws < HRM_MAX_WINDOW_SIZE)
				printf("%zu ", *ws);
		}
		printf("[hrtimer periods]\n");
	...
	printf("\tmin heart rate: %f\n", hrm_get_min_heart_rate(&monitor1, ws));
	printf("\tmax heart rate: %f\n", hrm_get_max_heart_rate(&monitor1, ws));
	printf("\tgoal scope: %zu\n", *ws);
	start = 1;
...
thread(void *arg)
{
	...
	cpu_set_t set;
	while (!start);
	hrm_attach(&monitor, proc_num + 1, false);
	running = 1;
	CPU_ZERO(&set);
