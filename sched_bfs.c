...
/*
 *	Our Modification are there below!
 */
	idle_cycles_offset++;
/*	if (idle_cycles_offset == INJECTION_IDLE_CYCLE_EACH_TIME){
...
/*********************************************/
	...
	if(is_init == 1){
		injection_value = INJECTION_IDLE_CYCLE_EACH_TIME_INIT;	
		injection_value_max = INJECTION_IDLE_CYCLE_EACH_TIME_INIT;
	}
	else{
		injection_value = INJECTION_IDLE_CYCLE_EACH_TIME_STANDARD;
		injection_value_max = INJECTION_IDLE_CYCLE_EACH_TIME_STANDARD_MAX;
	}
	
	if (unlikely(!queued_notrunning()) ||idle_cycles_offset == injection_value ){ //(( idle_cycles_offset >= injection_value) && (idle_cycles_offset <= injection_value_max) )) {
		/*
		...
		/*if(idle_cycles_offset == 20){
			idle_cycles_offset = 0;
			printk("sched -->it has gone");
		}
		else if (idle_cycles_offset == 2000){
			idle_cycles_offset = 0;
			printk("sched -->first time has gone");
		}*/
		idle_cycles_offset =0;
		next = idle;
